...
import com.application.food.lib.pagesview.OnImageRenderedListener;
import com.application.food.lib.pagesview.PagesProvider;
import com.application.food.lib.pagesview.RenderingException;
import com.application.food.lib.pagesview.Tile;
...
public class PDFPagesProvider extends PagesProvider {
	...
	private void setMaxCacheSize() {
		...
		int m = (int)(displaySize * 3f * 1.0001f);
...
//			m = maxMax - 20 * MB;
	...
	private static class BitmapCache {
		...
		Bitmap get(Tile k) {
			...
			if ((this.hits + this.misses) % 100 == 0 && (this.hits > 0 || this.misses > 0)) {
				Log.d("com.eni.icteam.doctra.pagecache", "hits: " + hits + ", misses: " + misses + ", hit ratio: " + (float)(hits) / (float)(hits+misses) +
	...
	private static class RendererWorker implements Runnable {
		...
		/**
		 * Thread's nation_layout routine.
